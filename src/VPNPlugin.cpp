...
{
    plugin_log_t openvpn_log = args->callbacks->plugin_log;
    ...
    try {
	ret->handle = (void**)new PluginContext(args->argv[1], openvpn_log);
    } catch (...) {
...
{
  PluginContext* const self = (PluginContext*)args->handle;
    if (version < OPENVPN_PLUGINv3_STRUCTVER)
	return self->openvpn_log(PLOG_ERR, PLUGIN_NAME, "Unexpected struct version"), OPENVPN_PLUGIN_FUNC_ERROR;
